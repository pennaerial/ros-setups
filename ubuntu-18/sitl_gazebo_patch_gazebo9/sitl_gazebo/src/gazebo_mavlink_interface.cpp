...
void GazeboMavlinkInterface::ImuCallback(ImuPtr& imu_message) {
  ...
  ignition::math::Pose3<double> T_W_I = model_->WorldPose();
  ignition::math::Vector3<double>pos_W_I = T_W_I.Pos();  // Use the models'world position for GPS and pressure alt.
  
  ignition::math::Quaternion<double> C_W_I;
  ...
  ignition::math::Quaternion<double> C_D_I(0.0, 0.0, declination);

  ignition::math::Vector3<double>mag_decl = C_D_I.RotateVectorReverse(mag_W_);

  // TODO replace mag_W_ in the line below with mag_decl

  ignition::math::Vector3<double>mag_I = C_W_I.RotateVectorReverse(mag_decl); // TODO: Add noise based on bais and variance like for imu and gyro
  ignition::math::Vector3<double>body_vel = C_W_I.RotateVectorReverse(model_->WorldLinearVel());
...
void GazeboMavlinkInterface::handle_control(double _dt)
{
    ...
    if (elevator_joint_ != NULL)
    {
      ...
      {
        double elevator_err =
         elevator_joint_->Position(0) - input_reference_[7];
