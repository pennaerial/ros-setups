  ...
  const ignition::math::Quaterniond &_q)
{
  ...
  // case zxy:
  ignition::math::Vector3<double> result = this->ThreeAxisRot(
...
void GimbalControllerPlugin::OnUpdate()
{
  ...
  else if (time > this->lastUpdateTime)
  {
    ...
    double rollLimited = ignition::math::clamp<double>(this->rollCommand,
      rDir*this->rollJoint->UpperLimit(0),
	  rDir*this->rollJoint->LowerLimit(0));
    double pitchLimited = ignition::math::clamp<double>(this->pitchCommand,
      pDir*this->pitchJoint->UpperLimit(0),
      pDir*this->pitchJoint->LowerLimit(0));
    double yawLimited = ignition::math::clamp<double>(this->yawCommand,
    ...
    ignition::math::Quaternion<double> currentAngleYPRVariable(
      this->imuSensor->Orientation().Euler());
    ignition::math::Vector3<double> currentAnglePRYVariable(
    ...
    {
      pitchError = ignition::math::clamp<double>(pitchError,
    ...
    {
      rollError = ignition::math::clamp<double>(rollError,
    ...
    {
      yawError = ignition::math::clamp<double>(yawError,
