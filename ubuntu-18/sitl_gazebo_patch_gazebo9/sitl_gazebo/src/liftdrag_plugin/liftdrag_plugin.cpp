...
void LiftDragPlugin::OnUpdate()
{
  ...
  ignition::math::Vector3<double> vel = this->link->WorldLinearVel(this->cp);
  ignition::math::Vector3<double> velI = vel;
  velI.Normalize();
  ...
  ignition::math::Vector3<double> spanwiseI = forwardI.Cross(upwardI).Normalize();

  const double minRatio = -1.0;
  const double maxRatio = 1.0;
  // check sweep (angle between velI and lift-drag-plane)
  double sinSweepAngle = ignition::math::clamp<double>(
  ...
  ignition::math::Vector3<double> velInLDPlane = vel - vel.Dot(spanwiseI)*velI;
  ...
  ignition::math::Vector3<double> liftI = spanwiseI.Cross(velInLDPlane);
  liftI.Normalize();

  // get direction of moment
  ignition::math::Vector3<double> momentDirection = spanwiseI;
  ...
  //   cos(theta) = a.Dot(b)
  double cosAlpha = ignition::math::clamp<double>(liftI.Dot(upwardI), minRatio, maxRatio);
  ...
  ignition::math::Vector3<double> lift = cl * q * this->area * liftI;
  ...
  ignition::math::Vector3<double> momentArm = pose.Rot().RotateVector(
